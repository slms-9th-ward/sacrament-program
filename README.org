...
Let's go through this line-by-line and examine what each bit does.
 ...
 - =@opening-hymn{68}= :: This creates an item in the program with the headline "opening hymn" in small caps. Underneath it puts the hymn number as well as the hymn name. (In this example, /A Might Fortress Is Our God/.) If you just put the hymn number in the braces, it will fetch the name automatically for you. If you put the hymn name (make sure you get the punctuation correct!) it will find the number for you!
